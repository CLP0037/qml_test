...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    client = new ClientStuff("localhost", 4001);//6547
...
{
    client->connect2host(10000);
}

void MainWindow::on_pushButton_send_clicked()
{
    QByteArray sendBuf;
    QString str = ui->lineEdit_message->text().trimmed();
    QStringList listbuf = str.split(' ');
    sendBuf.clear();
    bool ok;
    for(int i=0;i<listbuf.count();i++)
    {
        //sendBuf.append(listbuf[i].toUtf8());

        sendBuf.append(listbuf[i].toInt(&ok, 16));
    }
    client->tcpSocket->write(sendBuf);
